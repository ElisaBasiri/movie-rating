...
-- Create main tables if not exist (based on project schema)
...
-- 8. Movies (Removed description per schema)
...
FROM tmdb_selected s JOIN directors d ON d.name = s.director_name;
...
-- 10. Ratings (Removed rated_at per schema)
INSERT INTO movie_ratings (movie_id, score)
SELECT m.id, (floor(random() * 10) + 1)::INT
